...
//	data.p_pos = (M* in_position).xyz;
	...
	vec3 at = light_attenuation;
	float dist = length(p_light - data.p_pos)/100;
	float attenuation = 1.f / (at.x + at.y * dist + at.z * dist * dist);
	data.L_light = light_color * attenuation;
